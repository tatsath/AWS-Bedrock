...
Check the content generated above against the compliance document attached and suggest the changes. highlight the content that should be changed and why. Refer to the points in the content moderation policy attached for anything that should be changed.
...
def main():
    ...
    # st.set_page_config(page_title="NWG Brand Adherence Guardrail Platform")

    # Top header
    st.title('NWG Brand Adherence Guardrail Platform')

    # Section for creating content
    st.header('Campaign: Know your credit score')
    option = st.radio(
        "Which campaign would you like to generate and review content?",
        ('Generate using AI', 'Provide human-defined message'))

    if option == 'Generate using AI':
        if st.button('Submit'):
            st.write("AI-generated content will be shown here.")
    else:
        user_input = st.text_input("Enter your message here:")
        if st.button('Submit'):
            st.write("Your message:", user_input)

    # Section for Personalized content and review
    st.header("Personalized content")
    review_text = st.text_area("Review", height=150)
    if st.button('Submit Review'):
        st.write("Review submitted:", review_text)

    # Section to rewrite messages with advice
    st.header("Rewrite messages with advices")
    rewrite_text = st.text_area("Rewrite and Review", height=150)
    if st.button('Submit Rewrite'):
        st.write("Rewrite submitted:", rewrite_text)

    # To run the app, save the code in a file, for example, `app.py`, and run it using:
